...
function insertInitialData(mysqli $conn): void {
    ...
    if ($rowCarreras['total'] >= 0 || $rowEmailsAdmin['total'] >= 0 || 
        $rowAdmins['total'] >= 0 || $rowEgresados['total'] >= 0) {
        $carreras = [
            'Tec. en Programación',
            'Contador Público',
        ...
        foreach ($carreras as $nombre) {
            $stmtCarreras = mysqli_prepare($conn, "INSERT IGNORE INTO carreras (nombre) VALUES (?)");
        ...
        foreach ($admins as $a) {
            $stmtAdmins = mysqli_prepare($conn,
        ...
        $stmtEmailsAdmin = mysqli_prepare($conn,
        ...
        $stmtEgresados = mysqli_prepare($conn,
        ...
        foreach ($egresados as [$nombre, $apellido, $matricula, $email, $telefono, $carrera, $estado]) {
            mysqli_stmt_bind_param($stmtEgresados, 'ssissis', $nombre, $apellido, $matricula, $email, $telefono, $carrera, $estado);
